...
/**
 * WorkRecord Process 단계 리스너.
 *
 * <p>Processor(WorkRecordComputeProcessor)가 memberId → List&lt;WorkRecord&gt;를 변환하는
...
public class WorkRecordItemProcessListener implements ItemProcessListener<Long, List<WorkRecord>> {
    ...
    public void beforeProcess(Long memberId) {
        log.debug("[WorkRecord][PROCESS] memberId={} 처리 시작", memberId);
    }

    @Override
    public void afterProcess(Long memberId, List<WorkRecord> result) {
        if (result == null) {
            log.debug("[WorkRecord][PROCESS] memberId={} → 슬롯 없음 (skip)", memberId);
        } else {
            log.debug("[WorkRecord][PROCESS] memberId={} → WorkRecord {}건 생성", memberId, result.size());
        }
    }

    @Override
    public void onProcessError(Long memberId, Exception ex) {
        log.error("[WorkRecord][PROCESS] memberId={} 처리 오류 | cause={}: {}",
